...
export function ProcessChat({
  ...
  return (
    <div className="flex h-[520px] flex-col rounded-xl border border-white/10 bg-zinc-900/30">
      <ScrollArea className="flex-1 p-4">
        <div className="space-y-4">
          {messages.length === 0 ? (
            <div className="rounded-xl border border-white/10 bg-zinc-950/40 p-4 text-sm text-muted-foreground">
              <p className="font-medium text-zinc-200">Inicie a análise contextual</p>
              <p className="mt-1">
              ...
              <p className="mt-2 text-xs">
                Sugestão: <span className="text-zinc-200">“O que devo fazer diante deste despacho?”</span>
              </p>
            </div>
          ) : null}
          {messages.map((m) => (
            <div
              ...
              className={cn(
                ...
                m.role === "user"
                  ? "ml-8 bg-violet-600/15 text-zinc-100"
                  : "mr-8 border border-white/10 bg-zinc-950/60 text-zinc-200",
              )}
            >
              {m.role === "assistant" ? (
              ...
              {m.role === "assistant" ? (
                (() => {
                  ...
                  return (
                    <div className="mt-3 rounded-lg border border-white/10 bg-zinc-950/50 p-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {conf ? (
                            <span className="text-[11px] text-muted-foreground">
                              confiança jurídica: <span className="text-zinc-200">{conf}</span>
                            </span>
                          ) : null}
                        </div>
                      </div>
                      ...
                      {cits.length ? (
                        <ol className="space-y-1 text-[11px] text-zinc-300">
                          {cits.map((c) => (
                            <li key={String(c.ref)} className="leading-relaxed">
                              <span className="text-muted-foreground">[{String(c.ref)}]</span>{" "}
                              <span className="text-zinc-100">{String(c.label ?? "Fonte")}</span>
                              {c.type ? (

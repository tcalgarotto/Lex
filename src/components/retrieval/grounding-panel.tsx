...
export function GroundingPanel({ score, label, reason, candidates }: GroundingPanelProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-xl border bg-card/40 p-5 ring-1 backdrop-blur-sm",
        tone.ring,
      )}
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-baseline gap-2">
              ...
              <span
                className={cn(
                  "ml-1 rounded-md px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide",
                  tone.bg,
                  tone.text,
                )}
              >
...
function Tile({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div
      className={cn(
        "rounded-lg border bg-background/40 p-2.5 text-center",

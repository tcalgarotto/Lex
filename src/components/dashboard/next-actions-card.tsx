      ...
      <CardContent className="space-y-4">
        {visibleGroups.map((g) => (
          <section key={g.key} className="space-y-1.5">
            ...
            {g.items.length === 0 ? (
              <p className="rounded-md border border-dashed border-white/10 px-3 py-2 text-[11px] text-muted-foreground">
                {g.emptyText}
              </p>
            ) : (
              <ul className="space-y-1">
                {g.items.map((it) => (
                  <li key={it.id}>
                    <Link
                      href={it.href}
                      className="flex items-start justify-between gap-2 rounded-md border border-white/5 px-3 py-2 hover:bg-white/5"
                    >

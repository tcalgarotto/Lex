...
import { legalResearchSearchBodySchema } from "@/lib/legal-research/request-body";
...
export async function POST(req: Request) {
  ...
  const parsed = legalResearchSearchBodySchema.safeParse(json);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const cached = getCachedLegalResearch("search", hash);
  if (cached) {
  ...
  try {
    out = await getLegalResearchProvider().search(full);
  } catch (e) {
  ...
  setCachedLegalResearch("search", hash, out);

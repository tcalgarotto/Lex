 ...
 * Aliás REST de `CaseRequest` (pedidos) — compatível com `/requests`.
...
export async function GET(_req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const claims = await prisma.caseRequest.findMany({
    where: { caseId },
    orderBy: { ordinal: "asc" },
  });
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  await prisma.caseTimelineEvent.create({
    data: {
      ...
      payloadJson: { entity: "claim", action: "create", claimId: created.id },
  ...
  await recordCaseMutationActivity({
    workspaceId,
    kind: "case.claim.create",
    title: "Pedido criado",
    meta: { caseId, claimId: created.id },
  });

  return NextResponse.json({ ok: true, claim: created }, { status: 201 });
}
...
export async function PATCH(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const json = await req.json().catch(() => null);
  const parsed = patchSchema.extend({ id: z.string().cuid() }).safeParse(json);
  if (!parsed.success) {
    return NextResponse.json({ error: "Body inválido" }, { status: 400 });
  }
...
export async function DELETE(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const json = await req.json().catch(() => null);
  const parsed = z.object({ id: z.string().cuid() }).safeParse(json);
  if (!parsed.success) {
    return NextResponse.json({ error: "Body inválido" }, { status: 400 });
  }

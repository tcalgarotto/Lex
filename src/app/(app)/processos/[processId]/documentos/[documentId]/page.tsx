...
export default async function DocumentoPage({
 ...
 <CardContent>
 {doc.extractedText ? (
 <ScrollArea className="h-[520px] rounded-lg border border-[color:var(--border-default)] bg-[color:var(--surface-overlay-strong)] p-3">
 <pre className="whitespace-pre-wrap text-xs leading-relaxed text-[color:var(--text-primary)]">
 {doc.extractedText}
 </pre>
 </ScrollArea>
 ) : (
 ...
 <div className="rounded-lg border border-[color:var(--border-default)] bg-[color:var(--surface-overlay-strong)] p-3 text-xs text-muted-foreground">
 <p>
 {doc.totalChunks != null ? (
 <>
 total: <span className="text-[color:var(--text-primary)]">{doc.totalChunks}</span> · processados:{" "}
 <span className="text-[color:var(--text-primary)]">{doc.processedChunks ?? 0}</span>
 </>
 ) : (
 <>total: —</>
 )}
 ...
 <p className="truncate">
 qdrant: <span className="text-[color:var(--text-primary)]">{c.qdrantPointId ?? "—"}</span>
 </p>
 </div>
 </div>
 ))

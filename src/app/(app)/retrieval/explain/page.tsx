...
export default function RetrievalExplainPage() {
  ...
  const run = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/retrieval/explain?q=${encodeURIComponent(q)}&topK=8&useGraph=true&useRerank=true`);
      if (!res.ok) {
  ...
  return (
    <header className="space-y-3">
      ...
      <p className="text-base text-foreground">
        Veja <strong>como</strong> a IA recuperou cada fonte: pipeline cronometrado, scores discriminados,
        cadeia de citações, vigências, riscos jurídicos e síntese estratégica.
      </p>
      <div className="flex items-center gap-2">
  ...
  return (
    <div className="rounded-xl border border-dashed bg-card/30 p-8 text-center">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        O Lex executa BM25 + dense + grafo + rerank, aplica boosts hierárquicos/temporais e devolve
        explicabilidade completa por chunk.
      </p>
            ...
            <TabsContent value="chunks" className="space-y-3">
              {retrieval.chunks.length === 0 ? (
                ...
                retrieval.chunks.map((c, i) => (
                  <RetrievedChunkCard key={c.chunkId} chunk={c} rank={i + 1} />
                ))
              )}
            </TabsContent>

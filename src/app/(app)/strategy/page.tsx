                ...
                <CardSection title="Teses dominantes (agrupamento por âncora normativa)">
                  <ul className="space-y-2 text-xs">
                    {data.research.thesisGroups.slice(0, 8).map((t) => (
                      <li key={t.id} className="rounded-md border border-white/10 p-2">
...
function BadgeSeverity({ sev }: { sev: string }) {
  const tone =
    sev === "alta" ? "border-rose-500/40 text-rose-300" : sev === "media" ? "border-amber-500/40 text-amber-300" : "border-zinc-600 text-zinc-400";
  return (
...
function TrustUxOverview({ data }: { data: AnalyzePayload }) {
  ...
  return (
    <section
      ...
      className="rounded-xl border border-white/10 bg-zinc-950/40 p-4 space-y-4"
    >

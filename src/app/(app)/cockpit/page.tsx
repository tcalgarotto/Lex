        ...
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs uppercase tracking-wide text-muted-foreground">
              <Clock className="size-3.5" /> Timeline jurídica viva
            </div>
            <h1 className="text-2xl font-semibold">Cockpit operacional</h1>
            <p className="max-w-3xl text-sm text-muted-foreground">
              Alertas em tempo real (mudança jurisprudencial, tese enfraquecida, risco crescente),
              integrações (PJe / e-SAJ / Projudi / EPROC / Diário Oficial / e-mail / WhatsApp / calendário) e
              caixa de notificações operacionais — tudo auditável e multi-tenant.
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button onClick={sync} disabled={syncing} data-testid="cockpit-sync">
        ...
        <Tabs defaultValue="alerts">
          ...
          <TabsContent value="integrations" className="mt-4 space-y-2">
            {integrations.length === 0 ? (
              <EmptyHint label="Nenhuma integração configurada ainda. Use POST /api/integrations." />
            ) : (
              integrations.map((i) => (
                <article
                  key={i.id}

...
const SEVERITY_TONE: Record<CaseAlertSeverity, string> = {
 ...
 INFO: "border-[color:var(--border-default)] bg-[color:var(--surface-overlay)] text-[color:var(--text-secondary)]",
};

const STATUS_TONE: Record<IntegrationStatus, string> = {
 CONNECTED: "border-emerald-400/40 bg-emerald-400/10 text-emerald-200",
 DISCONNECTED: "border-[color:var(--border-default)] bg-[color:var(--surface-overlay)] text-[color:var(--text-secondary)]",
...
export default function CockpitPage() {
 ...
 <Clock className="size-3.5" /> Visão operacional (interna)
 </div>
 ...
 {checklist.length > 0 ? (
 <section
 className="rounded-xl border border-[color:var(--border-default)] bg-[color:var(--surface-overlay-strong)] p-4"
 ...
 <li
 key={item.id}
 className="flex items-start gap-2 rounded-md border border-[color:var(--border-subtle)] bg-[color:var(--surface-overlay)] px-3 py-2"
 >
 {item.ok ? (
 ...
 <article
 key={n.id}
 className="rounded-lg border border-[color:var(--border-default)] bg-[color:var(--surface-overlay-strong)] p-3"
...
function EmptyHint({ label }: { label: string }) {
 return (
 <p className="rounded-lg border border-dashed border-[color:var(--border-default)] bg-[color:var(--surface-overlay)] p-6 text-center text-xs text-muted-foreground">
 {label}
 </p>
 );
}
